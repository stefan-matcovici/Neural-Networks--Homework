...
if __name__ == "__main__":
    ...
    if parse_result.load:
        ...
        if parse_result.batch_size is not None:
            classifier.train_in_batches(train_set, valid_set, parse_result.learning_rate, parse_result.no_iterations,
                                        parse_result.batch_size, parse_result.adaline)
        else:
            classifier.train(train_set, valid_set, parse_result.learning_rate, parse_result.no_iterations,
                             parse_result.adaline)

        if parse_result.save:
            save("model-" + str(abs(int(math.log10(0.001)))) + "-" + str(parse_result.no_iterations) + (
                ("-" + str(parse_result.batch_size)) if parse_result.batch_size else "") + (
                     "-adaline" if parse_result.adaline else ""), classifier)

    print("Accuracy on test set: %f" % classifier.test(test_set))
